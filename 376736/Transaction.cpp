...
void Transaction::write_to_local(void *source, std::size_t size, void *target, uint16_t segment_id) {
...
bool Transaction::try_lock_write() {
    for (WriteData data: local_write_data) {
        LockWithVersion* versioned_lock = tm->memory_segments[data.segment_id]->get_vlock(data.data);
...
void Transaction::make_write() {
    for (WriteData wd: local_write_data) {
        LockWithVersion* versioned_lock = tm->memory_segments[wd.segment_id]->get_vlock(wd.data);
